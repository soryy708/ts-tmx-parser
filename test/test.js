...
describe("tilesets", function () {
  ...
  it("embedded tileset", function (done) {
    ...
    tmx.parseFile(target, function (err, map) {
      ...
      assert.strictEqual(
        map.tileSets[0].terrainTypes[0].properties.happiness,
        "yes",
      );
      ...
      assert.strictEqual(
        map.tileSets[0].tiles[0].terrain[0],
        map.tileSets[0].terrainTypes[3],
      );

      assert.strictEqual(
        map.tileSets[0].tiles[9].properties.brickcenter,
        "true",
      );
      done();
    });
  ...
  it("multiple tilesets", function (done) {
    ...
    tmx.parseFile(target, function (err, map) {
      ...
      assert.strictEqual(
        map.layers[0].tileAt(16, 12),
        map.tileSets[0].tiles[0],
      );
      assert.strictEqual(map.layers[0].tileAt(16, 12).gid, 1);
      assert.strictEqual(
        map.layers[0].tileAt(17, 12),
        map.tileSets[1].tiles[4],
      );
  ...
  it("animated tiles", function (done) {
    ...
    tmx.parseFile(target, function (err, map) {
      ...
      assert.strictEqual(
        map.tileSets[0].tiles[0].animations[5].duration,
        "100",
      );
      done();
    });
  ...
  it("objectgroups in tiles", function (done) {
    ...
    tmx.parseFile(target, function (err, map) {
      ...
      assert.strictEqual(
        map.tileSets[0].tiles[1].objectGroups[0].ellipse,
        true,
      );
...
function validateLemmingLevel3(map) {
  ...
  assert.strictEqual(map.layers[0].tileAt(35, 28), map.tileSets[0].tiles[5]);
